pipeline {
  ...
  environment {
    BASE_INSTANCE_ID  = 'i-REPLACE_WITH_BASE_APP_SERVER_ID'
    SECURITY_GROUP_ID = 'sg-REPLACE_WITH_YOUR_SG_ID'
    SUBNET_ID         = 'subnet-REPLACE_WITH_YOUR_SUBNET_ID'
    KEY_NAME          = 'ami-lab-key'
    ...
    stage('Bake AMI') {
      ...
      steps {
        script {
          ...
          def baseIp = sh(script: "aws ec2 describe-instances --instance-ids ${BASE_INSTANCE_ID} --region ${AWS_REGION} --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text", returnStdout: true).trim()
          
          sh """
            scp -i /var/lib/jenkins/.ssh/ami-lab-key.pem \
          ...
          def amiId = sh(script: "aws ec2 create-image --instance-id ${BASE_INSTANCE_ID} --name '${amiName}' --no-reboot --region ${AWS_REGION} --query 'ImageId' --output text", returnStdout: true).trim()
          env.AMI_ID = amiId
